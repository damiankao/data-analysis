...
gap_statistic = function(data, min_num_clusters = 1, max_num_clusters = 50, num_reference_bootstraps = 10) {
...
d = read.csv('regen.sense.filter20.outlierRemoved.sense.edgeRNormalized.mean.zscore',sep='\t',header=TRUE, row.name = 1)
pdf("gap.pdf")
gap_statistic(d)
dev.off()
